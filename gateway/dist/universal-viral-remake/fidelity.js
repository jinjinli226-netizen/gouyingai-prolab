const validStatuses = new Set(["matched", "missing", "contradicted", "uncertain"]);
export function createUniversalFidelityPrompt(contract, candidate, threshold) {
...
export function parseUniversalFidelityEvaluation(raw, contract, candidate, threshold) {
    const facts = collectUniversalFidelityFacts(contract, candidate);
    ...
    for (const item of Array.isArray(value.factResults) ? value.factResults : []) {
        ...
        if (!expected.has(factId) || results.has(factId) || !validStatuses.has(status))
            continue;
    ...
    const issues = factResults.filter((fact) => fact.status !== "matched").map((fact) => fact.issue || `${fact.id} 未通过视觉核对（${fact.status}）`);
    return {
...
export function buildUniversalFidelityCorrection(report) {
...
export function collectUniversalFidelityFacts(contract, candidate) {
    const facts = [];
    for (const unit of contract.timelineUnits) {
        for (const invariant of unit.structuralInvariants)
            facts.push({
                id: invariant.id,
        ...
        for (const event of unit.eventFacts)
            facts.push({
                id: event.id,
                kind: "event-fact",
        ...
        for (const [index, continuity] of unit.startContinuity.facts.entries())
            facts.push({
        ...
        for (const [index, continuity] of unit.endContinuity.facts.entries())
            facts.push({
    ...
    for (const entity of candidate.entityManifest ?? [])
        facts.push({
...
function parseJsonObject(raw) {
    ...
    if (start < 0 || end <= start)
        throw new Error("视觉忠实度评估没有返回 JSON 对象");
    const parsed = JSON.parse(normalized.slice(start, end + 1));
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
        throw new Error("视觉忠实度评估返回格式无效");
    return parsed;
}
function normalizeThreshold(value) {
    return Math.max(1, Math.min(100, Number.isFinite(value) ? value : 85));
}
function factWeight(fact) {
