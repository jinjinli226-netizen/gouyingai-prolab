...
export function createUniversalRemakeRouter(runs, coordinator, getUserId) {
    ...
    router.get("/", async (req, res) => {
        try {
            res.json({ data: await runs.list(getUserId(res), optionalString(req.query.canvasId) || undefined) });
        }
        catch (error) {
            sendError(res, error);
        }
    ...
    router.get("/:id", async (req, res) => {
        try {
            ...
            if (!run)
                return void res.status(404).json({ error: "通用复刻运行不存在" });
            res.json({ data: run });
        }
        catch (error) {
            sendError(res, error);
    ...
    function action(run) {
        return async (req, res) => {
            try {
                ...
                if (!await runs.get(getUserId(res), id))
                    return void res.status(404).json({ error: "通用复刻运行不存在" });
...
function parseCreateInput(body) {
    ...
    if (!Array.isArray(candidates) || !candidates.length || candidates.length > 1000)
        throw new RequestError("candidates 必须包含 1-1000 个完整候选计划");
    ...
    return {
        ...
        max_in_flight: integer(body?.maxInFlight ?? 2, "maxInFlight", 1, 20), candidates: candidates,
    };
}
function validateCandidate(value, index) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        throw new RequestError(`候选 ${index + 1} 无效`);
    const candidate = value;
    if (!requiredString(candidate.id, `候选 ${index + 1}.id`) || !Array.isArray(candidate.segments) || !candidate.segments.length)
        throw new RequestError(`候选 ${index + 1} 缺少分段`);
    if (!candidate.output || typeof candidate.output !== "object" || !["direct-video", "composed-video"].includes(String(candidate.output.kind)))
        throw new RequestError(`候选 ${index + 1} 输出计划无效`);
}
function requiredString(value, field) { const text = typeof value === "string" ? value.trim() : ""; if (!text)
    throw new RequestError(`${field} 必填`); return text; }
function optionalString(value) { return typeof value === "string" && value.trim() ? value.trim() : null; }
function pathParameter(value) { return Array.isArray(value) ? value[0] : value; }
function integer(value, field, min, max = Number.MAX_SAFE_INTEGER) { const number = Number(value); if (!Number.isInteger(number) || number < min || number > max)
    throw new RequestError(`${field} 必须在 ${min}-${max} 之间`); return number; }
class RequestError extends Error {
}
function sendError(res, error) { res.status(error instanceof RequestError ? 400 : 500).json({ error: error instanceof Error ? error.message : "通用复刻请求失败" }); }
