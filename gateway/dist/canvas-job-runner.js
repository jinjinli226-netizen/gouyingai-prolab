...
export class CanvasJobRunner {
    ...
    selectCandidates(candidates, available) {
        ...
        const activeByChannel = countBy([...this.active.values()].map((entry) => entry.job.channel_id).filter((value) => Boolean(value)));
            ...
            for (const [canvasId, queue] of [...queues]) {
                ...
                if (candidateIndex >= 0) {
                    ...
                    if (candidate.channel_id)
                        increment(activeByChannel, candidate.channel_id);
                    progressed = true;
                }
                if (!queue.length)
                    queues.delete(canvasId);
    ...
    withinLimits(job, activeByCapability, activeByChannel) {
        const capabilityLimit = this.settings.capabilityConcurrency[job.kind];
        if (capabilityLimit && (activeByCapability.get(job.kind) || 0) >= capabilityLimit)
            return false;
        if (job.channel_id) {
            const channelLimit = this.settings.channelConcurrency[job.channel_id];
            if (channelLimit && (activeByChannel.get(job.channel_id) || 0) >= channelLimit)
                return false;
        }
        return true;
    }
    async releaseClaim(job) {
    ...
    async startJob(claimed) {
        ...
        const update = async (patch) => {
            ...
            if (!updated)
                throw new Error(`画布任务 ${job.id} 不存在`);
            job = updated;
...
//# sourceMappingURL=canvas-job-runner.js.map