...
export function createVideoJobHandler(runtime) {
    return async ({ job, signal, update }) => {
        if (job.kind !== "video" && job.kind !== "viral-video")
            throw new Error(`任务 ${job.id} 不是视频任务`);
        if (!job.model_id || !job.channel_id)
            throw new Error("视频任务缺少固定的模型或渠道绑定");
...
export function createLocalVideoJobRuntime(store, overrides = {}) {
    ...
    return {
        ...
        submit: (context) => submitConfiguredVideo(context, async (job, uri) => {
            ...
            if (!artifact)
                throw new Error("视频参考素材不存在或无权访问");
        ...
        saveResult: async ({ job, channel, result, signal }) => {
            if (result.storageKey?.startsWith("canvas-artifact:"))
                return result;
...
export function createSupabaseVideoJobRuntime(admin, overrides = {}) {
    return {
        ...
        submit: (context) => submitConfiguredVideo(context, async (job, uri) => {
            ...
            if (error || !data)
                throw new Error(error?.message || "视频参考素材下载失败");
        ...
        saveResult: async ({ job, channel, result, signal }) => {
            ...
            if (signed.error || !signed.data?.signedUrl)
                throw new Error(signed.error?.message || "视频结果签名失败");
...
async function submitConfiguredVideo({ job, model, channel }, resolveArtifact) {
    ...
    if (channel.api_format === "autodl_comfyui") {
        ...
        if (!response.ok || task.status === "failed")
            return { status: "failed", upstreamTaskId: task.upstreamTaskId, error: task.error || `AutoDL 视频提交失败（${response.status}）` };
        if (task.status === "completed" && task.resultUrl)
            return { status: "completed", upstreamTaskId: task.upstreamTaskId, result: { url: task.resultUrl, mimeType: "video/mp4" } };
    ...
    if (!response.ok)
        return { status: "failed", error: readUpstreamError(payload) || `视频提交失败（${response.status}）` };
    return normalizeVideoState(payload);
}
async function resolveVideoInputArtifacts(job, input, resolveArtifact) {
    const resolveValues = (values) => Promise.all(values.map((value) => {
        if (!value.startsWith("canvas-artifact:"))
...
async function pollConfiguredVideo({ channel, upstreamTaskId }) {
    if (channel.api_format === "autodl_comfyui") {
        const { response, task } = await queryAutoDlWorkflow(channel.base_url, decryptSecret(channel.key_ciphertext), upstreamTaskId);
        if (!response.ok || task.status === "failed")
            return { status: "failed", upstreamTaskId, error: task.error || `AutoDL 视频查询失败（${response.status}）` };
        if (task.status === "completed" && task.resultUrl)
            return { status: "completed", upstreamTaskId, result: { url: task.resultUrl, mimeType: "video/mp4" } };
    ...
    const payload = await response.json().catch(() => null);
    if (!response.ok)
        return { status: "failed", upstreamTaskId, error: readUpstreamError(payload) || `视频查询失败（${response.status}）` };
...
function normalizeVideoResolution(value) {
    if (value === "low")
        return "480p";
    if (["auto", "high", "medium"].includes(value))
        return "768p";
    const resolution = value.replace(/p$/i, "") || "768";
    return `${resolution}p`;
}
async function buildVideoForm(modelName, input) {
    ...
    for (const [index, value] of input.referenceImages.entries())
        form.append(referenceImageField, dataUrlFile(value, `reference-${index}`) || value);
    for (const [index, value] of input.referenceVideos.entries())
        form.append("reference_videos", dataUrlFile(value, `reference-video-${index}`) || value);
    for (const [index, value] of input.referenceAudios.entries())
        form.append("reference_audios", dataUrlFile(value, `reference-audio-${index}`) || value);
    return form;
}
...
function normalizeVideoState(payload, fallbackTaskId) {
    ...
    if (url)
        return { status: "completed", upstreamTaskId, result: { url, mimeType: "video/mp4" } };
    if (["failed", "failure", "cancelled", "canceled", "expired"].includes(status))
        return { status: "failed", upstreamTaskId, error: readUpstreamError(value) || "视频生成失败" };
    if (["completed", "succeeded", "success"].includes(status))
        return { status: "failed", upstreamTaskId, error: "视频任务成功但没有返回视频 URL" };
    if (upstreamTaskId)
        return { status: "pending", upstreamTaskId };
    return { status: "failed", error: readUpstreamError(value) || "视频接口没有返回任务 ID" };
}
async function downloadVideoResult(channel, result, signal) {
    let url;
    try {
...
function firstString(...values) {
    return values.find((value) => typeof value === "string" && Boolean(value)) || "";
}
function delay(milliseconds, signal) {
    return new Promise((resolve, reject) => {
        if (signal.aborted)
            return void reject(new Error("视频任务执行已停止"));
...
//# sourceMappingURL=video-job-handler.js.map