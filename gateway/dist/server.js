...
export function createGatewayApp(context) {
    ...
    app.get("/v1/models", requireAuth, async (req, res) => {
        ...
        const published = unambiguousPublishedModels((models || []).filter((model) => enabledChannelIds.has(model.channel_id)));
        const filtered = capability ? published.filter((model) => model.capability === capability) : published;
        res.json({ data: filtered.map((model) => toPublicModel(model, (channels || []).find((channel) => channel.id === model.channel_id))) });
    });
    ...
    app.all("/v1/*splat", requireAuth, express.raw({ type: () => true, limit: "100mb" }), async (req, res) => {
        const videoTaskMatch = req.path.match(/^\/v1\/videos\/([^/]+)(?:\/content)?$/);
        if (req.method === "GET" && videoTaskMatch) {
            const videoTaskId = videoTaskMatch[1];
            const task = memoryTasks.get(videoTaskId) || (await admin.from("gouyingai_gateway_tasks").select("*").eq("task_id", videoTaskId).maybeSingle()).data;
            if (task && task.user_id === res.locals.user.id && task.capability === "video") {
                ...
                const taskChannel = (channels || []).find((item) => item.id === task.channel_id);
                if (taskChannel) {
                    ...
                    if (poll.headers.get("content-type"))
                        res.setHeader("content-type", poll.headers.get("content-type") || "");
                    res.send(pollBody);
                    return;
                }
            }
        ...
        if (!modelName)
            return void res.status(400).json({ error: "请求体缺少 model 字段" });
        const [models, channels] = await Promise.all([
            admin.from("gouyingai_models").select("*"),
            admin.from("gouyingai_channels").select("*"),
        ]);
        const model = pickModelByName((models.data || []), modelName);
        if (!model)
            return void res.status(404).json({ error: `模型 ${modelName} 未发布或不存在` });
        const requestedCapability = parseModelCapability(req.headers[MODEL_CAPABILITY_HEADER]);
        if (!requestedCapability)
            return void res.status(400).json({ error: "请求缺少有效的模型能力声明" });
        if (model.capability !== requestedCapability)
            return void res.status(409).json({ error: `模型 ${modelName} 不支持 ${requestedCapability} 请求` });
        const channel = pickChannelById((channels.data || []), model.channel_id);
        if (!channel)
            return void res.status(502).json({ error: "模型所在渠道不可用" });
        ...
        for (const [key, value] of Object.entries(req.headers)) {
            if (!value || ["host", "content-length", "connection", "transfer-encoding", "expect", MODEL_CAPABILITY_HEADER].includes(key))
                continue;
...
async function recordUsage(admin, userId, modelId, capability, status, tokens) {
    try {
