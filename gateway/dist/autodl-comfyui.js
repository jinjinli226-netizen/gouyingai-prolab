export function buildAutoDlWorkflowRequest(options, input) {
    ...
    const values = { ...config.constants, ...indexedImages, ...indexedAudios, prompt, duration, resolution, referenceImages, referenceAudios };
    const body = resolveTemplate(config.requestTemplate, values);
    if (!isObject(body))
        throw new Error("AutoDL requestTemplate 必须生成 JSON 对象");
    return { workflowId: config.workflowId, body };
}
export function normalizeAutoDlTask(payload) {
    ...
    if (status === "RUNNING")
        return { upstreamTaskId, status: "in_progress" };
    if (status === "QUEUED" || upstreamTaskId)
        return { upstreamTaskId, status: "queued" };
    const code = stringValue(envelope.code).toUpperCase();
    if (code && code !== "SUCCESS")
        return { status: "failed", error: error || "AutoDL 请求失败" };
    return { status: "failed", error: error || "AutoDL 返回了无法识别的任务状态" };
}
export async function parseUnifiedVideoInput(raw, contentType) {
    ...
    const referenceImages = Array.isArray(body.reference_images) ? body.reference_images.filter((value) => typeof value === "string") : [];
    const referenceAudios = Array.isArray(body.reference_audios) ? body.reference_audios.filter((value) => typeof value === "string") : [];
    return {
...
export function autoDlCreateUrl(baseUrl, workflowId) {
    return `${trimBaseUrl(baseUrl)}/comfyui_workflow/${encodeURIComponent(workflowId)}`;
}
export function autoDlResultUrl(baseUrl, taskId) {
    return `${trimBaseUrl(baseUrl)}/comfyui_workflow/result/${encodeURIComponent(taskId)}`;
}
export async function submitAutoDlWorkflow(baseUrl, token, workflow) {
...
export async function queryAutoDlWorkflow(baseUrl, token, taskId) {
    const response = await fetch(autoDlResultUrl(baseUrl, taskId), { headers: { authorization: token } });
    return { response, task: normalizeAutoDlTask(await response.json().catch(() => null)) };
}
function readAutoDlModelOptions(options) {
    const autodl = isObject(options.autodl) ? options.autodl : {};
    const workflowId = stringValue(autodl.workflowId).trim();
    if (!workflowId)
        throw new Error("AutoDL 模型缺少 workflowId");
    if (!isObject(autodl.requestTemplate))
        throw new Error("AutoDL 模型缺少 requestTemplate");
    return {
        workflowId,
...
function mappedValue(map, key, label) {
    if (Object.prototype.hasOwnProperty.call(map, key))
        return map[key];
    throw new Error(`AutoDL 不支持${label} ${key || "（空）"}`);
}
function mappedResolution(map, resolution, size) {
...
function resolveTemplate(value, values) {
    if (Array.isArray(value))
        return value.map((item) => resolveTemplate(item, values));
    if (isObject(value)) {
        const entries = [];
    ...
    if (exact)
        return templateValue(values, exact[1]);
    return value.replace(/\{\{([A-Za-z][A-Za-z0-9_]*)\}\}/g, (_match, name) => String(templateValue(values, name)));
}
function templateValue(values, name) {
    if (!Object.prototype.hasOwnProperty.call(values, name))
        throw new Error(`AutoDL requestTemplate 使用了未知占位符 {{${name}}}`);
    return values[name];
}
...
async function formValueToMediaInput(value) {
    if (typeof value === "string")
        return value;
...
//# sourceMappingURL=autodl-comfyui.js.map