...
export async function materializeViralBatchWindow(batchId, batches, jobs) {
    ...
    const manifests = Array.isArray(batch.input.manifests) ? batch.input.manifests : [];
    const baseVideoInput = batch.input.baseVideoInput && typeof batch.input.baseVideoInput === "object" && !Array.isArray(batch.input.baseVideoInput) ? batch.input.baseVideoInput : {};
...
export async function reconcileViralBatch(batchId, batches, jobs) {
    const batch = await batches.getInternal(batchId);
    if (!batch)
        return null;
    const children = (await jobs.list(batch.user_id, { batchId: batch.id })).filter((job) => job.kind === "viral-video");
    ...
    children.forEach((job) => {
        if (job.status === "queued" || job.status === "leased" || job.status === "submitting")
...
export class ViralBatchCoordinator {
    ...
    intervalMs;
    timer = null;
    constructor(batches, jobs, intervalMs = 1000) {
...
//# sourceMappingURL=viral-batch-coordinator.js.map