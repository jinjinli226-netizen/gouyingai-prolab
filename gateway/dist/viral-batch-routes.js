...
export function createViralBatchRouter(batches, jobs, getUserId) {
    ...
    router.get("/", async (req, res) => {
        try {
            res.json({ data: (await batches.list(getUserId(res), optionalString(req.query.canvasId) || undefined)).map(toPublicViralBatch) });
        }
        catch (error) {
            sendError(res, error);
        }
    ...
    router.get("/:id/candidates", async (req, res) => {
        try {
            ...
            if (!batch)
                return void res.status(404).json({ error: "爆款复刻批次不存在" });
...
export function toPublicViralBatch(batch) {
    return {
...
function parseCreateInput(body) {
    ...
    if (!Array.isArray(manifests) || manifests.length < candidateCount)
        throw new RequestError("input.manifests 必须覆盖全部候选");
...
function optionalNonNegativeNumber(value, field) { if (value === undefined || value === null || value === "")
    return null; const number = Number(value); if (!Number.isFinite(number) || number < 0)
    throw new RequestError(`${field} 必须是非负数`); return number; }
class RequestError extends Error {
}
function sendError(res, error) { res.status(error instanceof RequestError ? 400 : 500).json({ error: error instanceof Error ? error.message : "爆款复刻批次请求失败" }); }
//# sourceMappingURL=viral-batch-routes.js.map