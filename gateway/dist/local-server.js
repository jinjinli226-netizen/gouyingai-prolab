...
export function createLocalGatewayApp(store) {
    const app = express();
    ...
    app.all("/v1/*splat", express.raw({ type: () => true, limit: "100mb" }), async (req, res) => {
    ...
    app.post("/admin/channels", (req, res) => {
        ...
        if (!String(name || "").trim() || !String(base_url || "").trim())
            return void res.status(400).json({ error: "渠道名称和 Base URL 必填" });
    ...
    app.patch("/admin/channels/:id", (req, res) => {
        ...
        if (req.body?.base_url)
            patch.base_url = String(req.body.base_url).trim();
        if (req.body?.api_format)
            patch.api_format = req.body.api_format;
        if (typeof req.body?.enabled === "boolean")
            patch.enabled = req.body.enabled;
        if (req.body?.api_key)
            patch.key_ciphertext = encryptSecret(String(req.body.api_key));
        const channel = store.updateChannel(req.params.id, patch);
        if (!channel)
            return void res.status(404).json({ error: "渠道不存在" });
        res.json({ data: { ...channel, key_ciphertext: maskSecret(channel.key_ciphertext) } });
    });
    ...
    app.patch("/admin/models/:id", (req, res) => {
        ...
        if (!model)
            return void res.status(404).json({ error: "模型不存在" });
        res.json({ data: model });
    });
