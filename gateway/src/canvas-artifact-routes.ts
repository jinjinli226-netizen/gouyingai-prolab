...
type ArtifactSaver = (userId: string, canvasId: string, file: File, bytes: Buffer, checksum: string) => Promise<SavedArtifact>;

export function createLocalCanvasArtifactRouter(store: LocalStore, getUserId: (res: Response) => string = () => "local") {
    ...
    router.get("/content", (req: Request, res: Response) => {
        ...
        if (!artifact) return void res.status(404).json({ error: "画布素材不存在" });
    ...
    router.get("/url", (req: Request, res: Response) => {
        const canvasId = String(req.query.canvasId || "");
        const uri = String(req.query.uri || "");
        if (!store.readCanvasArtifact(getUserId(res), canvasId, uri)) return void res.status(404).json({ error: "画布素材不存在" });
...
export function createSupabaseCanvasArtifactRouter(admin: SupabaseClient, getUserId: (res: Response) => string) {
    const router = createCanvasArtifactRouter(getUserId, async (userId, canvasId, file, bytes, checksum) => {
        const id = randomUUID();
    ...
    router.get("/url", async (req: Request, res: Response) => {
        try {
            ...
            if (!canvasId || !isOwnedCanvasArtifactPath(getUserId(res), canvasId, path)) return void res.status(404).json({ error: "画布素材不存在" });
            const signed = await admin.storage.from(MEDIA_BUCKET).createSignedUrl(path, 24 * 60 * 60);
            if (signed.error || !signed.data?.signedUrl) throw new Error(signed.error?.message || "素材签名失败");
...
function createCanvasArtifactRouter(getUserId: (res: Response) => string, saveArtifact: ArtifactSaver) {
    const router = express.Router();
    router.post("/", express.raw({ type: () => true, limit: `${MAX_ARTIFACT_BYTES + 1024 * 1024}b` }), async (req: Request, res: Response) => {
        try {
            const contentType = String(req.headers["content-type"] || "");
            if (!contentType.toLowerCase().includes("multipart/form-data")) return void res.status(400).json({ error: "请使用 multipart/form-data 上传素材" });
            const raw = Buffer.isBuffer(req.body) ? req.body : Buffer.alloc(0);
            const request = new globalThis.Request("http://gateway.local/v1/canvas-artifacts", { method: "POST", headers: { "content-type": contentType }, body: raw as unknown as BodyInit });
            const form = await request.formData();
            const canvasId = stringValue(form.get("canvasId")).trim();
            const file = form.get("file");
            if (!canvasId) return void res.status(400).json({ error: "canvasId 必填" });
            if (!(file instanceof File)) return void res.status(400).json({ error: "file 必填" });
            if (!isAllowedMediaType(file.type)) return void res.status(415).json({ error: "只允许上传图片、视频或音频素材" });
            if (file.size <= 0 || file.size > MAX_ARTIFACT_BYTES) return void res.status(413).json({ error: "素材大小必须在 100MB 以内" });
...
export function isOwnedCanvasArtifactPath(userId: string, canvasId: string, path: string) {
...
export function canvasArtifactStoragePath(uri: string) {
