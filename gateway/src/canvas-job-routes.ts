...
import type { CanvasJobCreateInput, CanvasJobKind, CanvasJobRepository, CanvasJobStatus } from "./types.js";

const kinds = new Set<CanvasJobKind>(["text", "image", "video", "audio", "viral-analysis", "viral-plan", "viral-video", "viral-quality"]);
const statuses = new Set<CanvasJobStatus>(["queued", "leased", "submitting", "running", "succeeded", "failed", "cancel_requested", "cancelled"]);

export function createCanvasJobRouter(repository: CanvasJobRepository, getUserId: (res: Response) => string) {
    ...
    router.get("/", async (req: Request, res: Response) => {
        try {
            const status = req.query.status ? String(req.query.status) as CanvasJobStatus : undefined;
            if (status && !statuses.has(status)) throw new RequestError("无效的任务状态");
    ...
    router.get("/:id", async (req: Request, res: Response) => {
        try {
            ...
            if (!job) return void res.status(404).json({ error: "画布任务不存在" });
    ...
    router.post("/:id/cancel", async (req: Request, res: Response) => {
        try {
            const job = await repository.get(getUserId(res), pathParameter(req.params.id));
            if (!job) return void res.status(404).json({ error: "画布任务不存在" });
            if (["succeeded", "failed", "cancelled"].includes(job.status)) return void res.json({ data: toPublicCanvasJob(job) });
            const nextStatus: CanvasJobStatus = job.status === "queued" ? "cancelled" : "cancel_requested";
            if (!canTransitionCanvasJob(job.status, nextStatus)) return void res.status(409).json({ error: "当前任务状态不可取消" });
    ...
    router.post("/:id/retry", async (req: Request, res: Response) => {
        try {
            ...
            if (!original) return void res.status(404).json({ error: "画布任务不存在" });
...
function parseCreateInput(body: Record<string, unknown> | undefined): CanvasJobCreateInput {
    const kind = requiredString(body?.kind, "kind") as CanvasJobKind;
    if (!kinds.has(kind)) throw new RequestError("无效的任务类型");
...
function requiredString(value: unknown, field: string) {
    ...
    if (!normalized) throw new RequestError(`${field} 必填`);
    return normalized;
}
...
function pathParameter(value: string | string[]) {
    return Array.isArray(value) ? value[0] : value;
}

function positiveInteger(value: unknown, field: string) {
    const normalized = Number(value);
    if (!Number.isInteger(normalized) || normalized <= 0) throw new RequestError(`${field} 必须是正整数`);
    return normalized;
}
...
function optionalNonNegativeInteger(value: unknown, field: string) {
    ...
    const normalized = Number(value);
    if (!Number.isInteger(normalized) || normalized < 0) throw new RequestError(`${field} 必须是非负整数`);
    return normalized;
}
