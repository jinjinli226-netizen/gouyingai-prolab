...
import type { CanvasJob, Channel, Model } from "../types.js";
...
type RuntimeDependencies = {
    loadModel: (id: string) => Promise<Model | null>;
    loadChannel: (id: string) => Promise<Channel | null>;
    resolveArtifact: (job: CanvasJob, uri: string) => Promise<string>;
    saveOutput: (job: CanvasJob, bytes: Buffer, mimeType: string, name: string) => Promise<{ url: string; storageKey: string }>;
    isGenerationCurrent: (job: CanvasJob) => Promise<boolean>;
};

export function createLocalCapabilityRuntimes(store: LocalStore): RuntimeSet {
    ...
    return createCapabilityRuntimes({
        ...
        resolveArtifact: async (job, uri) => {
            ...
            if (!artifact) throw new Error("画布参考素材不存在或无权访问");
...
export function createSupabaseCapabilityRuntimes(admin: SupabaseClient): RuntimeSet {
    return createCapabilityRuntimes({
        loadModel: async (id) => {
        ...
        resolveArtifact: async (job, uri) => {
            ...
            if (!isOwnedCanvasArtifactPath(job.user_id, job.canvas_id, path)) throw new Error("画布参考素材不存在或无权访问");
            const { data, error } = await admin.storage.from("gouyingai-media").download(path);
            if (error || !data) throw new Error(error?.message || "画布参考素材下载失败");
        ...
        saveOutput: async (job, bytes, mimeType, name) => {
            ...
            if (signed.error || !signed.data?.signedUrl) throw new Error(signed.error?.message || "生成结果签名失败");
...
function createCapabilityRuntimes(dependencies: RuntimeDependencies): RuntimeSet {
    const shared = {
...
async function generateText(job: CanvasJob, model: Model, channel: Channel, signal: AbortSignal, resolveArtifact: RuntimeDependencies["resolveArtifact"]) {
    ...
    if (channel.api_format === "gemini") {
        ...
        for (const reference of references) {
            ...
            if (data) parts.push({ inlineData: { mimeType: data.mimeType, data: data.bytes.toString("base64") } });
        }
        ...
        if (!response.ok) throw new Error(readError(payload) || `文本生成失败（${response.status}）`);
        ...
        if (!combined) throw new Error("文本接口没有返回内容");
    ...
    if (!text) throw new Error("文本接口没有返回内容");
    return { text };
}

async function generateImage(job: CanvasJob, model: Model, channel: Channel, signal: AbortSignal, resolveArtifact: RuntimeDependencies["resolveArtifact"], saveOutput: RuntimeDependencies["saveOutput"]) {
    const prompt = stringValue(job.input.prompt);
    ...
    if (channel.api_format === "gemini") {
        ...
        const payload = await response.json().catch(() => null);
        if (!response.ok) throw new Error(readError(payload) || `图片生成失败（${response.status}）`);
        ...
        if (encoded) bytes = Buffer.from(encoded, "base64");
            ...
            if (!media.ok) throw new Error(`生成图片下载失败（${media.status}）`);
...
async function generateAudio(job: CanvasJob, model: Model, channel: Channel, signal: AbortSignal, saveOutput: RuntimeDependencies["saveOutput"]) {
    if (channel.api_format === "gemini") throw new Error("Gemini 渠道不支持音频生成");
    ...
    if (!response.ok) throw new Error(readError(await response.json().catch(() => null)) || `音频生成失败（${response.status}）`);
...
async function resolveArtifactList(job: CanvasJob, value: unknown, resolver: RuntimeDependencies["resolveArtifact"]) {
    const values = Array.isArray(value) ? value.filter((item): item is string => typeof item === "string") : [];
    return Promise.all(values.map((item) => {
        if (!item.startsWith("canvas-artifact:")) throw new Error("后台任务只接受已上传到当前画布的私有参考素材");
        return resolver(job, item);
    }));
}

function buildImageEditForm(modelName: string, prompt: string, input: Record<string, unknown>, references: string[]) {
    const form = new FormData();
    form.append("model", modelName);
    form.append("prompt", prompt);
    if (stringValue(input.size)) form.append("size", stringValue(input.size));
    if (stringValue(input.quality)) form.append("quality", stringValue(input.quality));
    const imageField = references.length > 1 ? "image[]" : "image";
    references.forEach((reference, index) => {
        const data = parseDataUrl(reference);
        if (data) form.append(imageField, new File([data.bytes], `reference-${index}.png`, { type: data.mimeType }));
        else form.append(imageField, reference);
    });
    return form;
}

function readTextResult(payload: unknown) {
...
function upstreamJsonHeaders(channel: Channel) {
    return { authorization: `Bearer ${decryptSecret(channel.key_ciphertext)}`, "content-type": "application/json" };
}

function geminiUrl(baseUrl: string, modelName: string) {
...
async function isSupabaseGenerationCurrent(admin: SupabaseClient, job: CanvasJob) {
