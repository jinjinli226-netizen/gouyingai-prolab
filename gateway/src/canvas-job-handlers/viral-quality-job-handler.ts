...
export function createViralQualityJobHandler(): CanvasJobHandler {
    return async ({ job }) => {
        if (job.kind !== "viral-quality") throw new Error(`任务 ${job.id} 不是复刻质量任务`);
        const metrics = record(job.input.metrics, "metrics");
        const scores = Object.fromEntries(Object.keys(weights).map((key) => [key, score(metrics[key], key)])) as Record<keyof typeof weights, number>;
        ...
        if (scores.productFidelity < 80) issues.push("商品还原不足");
        if (scores.identityConsistency < 80) issues.push("人物一致性不足");
        const quality = {
...
function record(value: unknown, field: string) { if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`${field} 必须是对象`); return value as Record<string, unknown>; }
function score(value: unknown, field: string) { const number = Number(value); if (!Number.isFinite(number)) throw new Error(`${field} 必须是分数`); return Math.max(0, Math.min(100, number)); }
