...
export type UniversalResolvedArtifact = { filePath: string; mimeType: string };
export type UniversalArtifactResolver = (request: { userId: string; canvasId: string; uri: string }) => Promise<UniversalResolvedArtifact | null>;
...
export type UniversalCompositionPlan = {
    ...
    transitions: Array<{ afterSegmentIndex: number; durationSeconds: number }>;
};

export type UniversalProcessResult = { exitCode: number; stderr: string };
...
export type UniversalFidelityContract = {
    ...
    timelineUnits: Array<{
        ...
        direction: string;
        structuralInvariants: Array<{ id: string; dimension: string; description: string; importance: UniversalFidelityImportance }>;
        eventFacts: Array<{
            id: string;
            family: string;
            dimension: string;
        ...
        startContinuity: { facts: Array<{ dimension: string; description: string; participantPlaceholderIds: string[] }> };
        endContinuity: { facts: Array<{ dimension: string; description: string; participantPlaceholderIds: string[] }> };
    }>;
};
...
export type UniversalFidelityEvaluation = {
    ...
    issues: string[];
    factResults: Array<UniversalFidelityFact & { status: "matched" | "missing" | "contradicted" | "uncertain"; issue: string }>;
};
...
export type UniversalRunCandidateManifest = {
    ...
    output: Record<string, unknown> & { kind: "direct-video" | "composed-video" };
    entityManifest?: Array<{ placeholderId: string; identityFacts: string; physicalInstanceCount: number; [key: string]: unknown }>;
    [key: string]: unknown;
};
...
export type UniversalRemakeRunCreateInput = Pick<UniversalRemakeRun,
    "canvas_id" | "target_node_id" | "generation_revision" | "client_request_id" | "template_id" | "model_id" | "channel_id" | "max_in_flight"
> & Partial<Pick<UniversalRemakeRun,
    "fidelity_model_id" | "fidelity_channel_id" | "source_video_artifact_uri" | "source_reference_asset_ids" | "fidelity_contract"
    | "fidelity_threshold" | "max_fidelity_retries"
