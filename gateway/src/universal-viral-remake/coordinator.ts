...
import type {
    UniversalRemakeRun,
    UniversalRemakeRunRepository,
    UniversalRunCandidate,
} from "./types.js";

export type UniversalCompositionPort = {
    compose(run: UniversalRemakeRun, candidate: UniversalRunCandidate, segmentArtifactUris: string[]): Promise<{ artifactUri: string }>;
    extractContinuationFrame(run: UniversalRemakeRun, candidate: UniversalRunCandidate, segmentArtifactUri: string): Promise<{ artifactUri: string }>;
};
...
export class UniversalRemakeCoordinator {
    ...
    private reconciliations = new Map<string, Promise<UniversalRemakeRun | null>>();

    constructor(
        private runs: UniversalRemakeRunRepository,
        private jobs: CanvasJobRepository,
        private composition: UniversalCompositionPort,
    ...
    async runOnce() {
        for (const run of await this.runs.listActive()) await this.reconcile(run.id);
    }

    async reconcile(runId: string): Promise<UniversalRemakeRun | null> {
        ...
        try {
            ...
            if (this.reconciliations.get(runId) === reconciliation) this.reconciliations.delete(runId);
        }
    }

    private async reconcileUnlocked(runId: string): Promise<UniversalRemakeRun | null> {
        let run = await this.runs.getInternal(runId);
        if (!run || run.status === "cancelled" || run.status === "completed" || run.status === "failed") return run;
        ...
        if (run.status !== "paused") {
            ...
            for (const candidate of run.candidates.filter((item) => item.status === "queued").slice(0, available)) {
                ...
                if (current) await this.advanceCandidate(run, current);
                available -= 1;
    ...
    async retryComposition(runId: string, candidateIndex: number) {
        const run = await this.runs.getInternal(runId);
        const candidate = run?.candidates.find((item) => item.index === candidateIndex);
        if (!run || !candidate) throw new Error("通用复刻候选不存在");
        if (candidate.status !== "failed" || candidate.failure_stage !== "composition") throw new Error("只有合成失败的候选可以单独重试合成");
    ...
    async pause(runId: string) { return this.runs.update(runId, { status: "paused", finished_at: null }); }
    async resume(runId: string) { return this.runs.update(runId, { status: "running", finished_at: null }); }
    ...
    private async advanceCandidate(run: UniversalRemakeRun, candidate: UniversalRunCandidate) {
        if (candidate.status === "composing") return this.compose(run, candidate);
        const segmentIndex = candidate.segment_artifact_uris.length;
        const currentJobId = candidate.segment_job_ids[segmentIndex];
        if (!currentJobId) {
            if (segmentIndex >= candidate.manifest.segments.length) return this.finishOrCompose(run, candidate);
        ...
        if (!refreshed) return;
        if (artifacts.length >= refreshed.manifest.segments.length) return this.finishOrCompose(run, refreshed);
        return this.createSegmentJob(run, refreshed, artifacts.length);
    }

    private async createSegmentJob(run: UniversalRemakeRun, candidate: UniversalRunCandidate, segmentIndex: number) {
        const segment = candidate.manifest.segments[segmentIndex];
        if (!segment) return;
        let continuationFrameArtifactUri: string | null = null;
        if (segmentIndex > 0 && typeof segment.continuationFrameFromSegmentId === "string") {
            ...
            if (!continuationFrameArtifactUri) {
                ...
                try {
                    ...
                    if (!extracted.artifactUri.startsWith("canvas-artifact:")) throw new Error("续接帧提取器没有返回托管画布素材");
    ...
    private async failSegment(run: UniversalRemakeRun, candidate: UniversalRunCandidate, error: string) {
        await this.runs.updateCandidate(run.id, candidate.index, { status: "failed", failure_stage: "segment", error });
    }

    private async finishOrCompose(run: UniversalRemakeRun, candidate: UniversalRunCandidate) {
        ...
        const refreshed = (await this.runs.getInternal(run.id))?.candidates.find((item) => item.index === candidate.index);
        if (refreshed) await this.compose(run, refreshed);
    }

    private async compose(run: UniversalRemakeRun, candidate: UniversalRunCandidate) {
        try {
            const result = await this.composition.compose(run, candidate, candidate.segment_artifact_uris);
            if (!result.artifactUri.startsWith("canvas-artifact:")) throw new Error("合成器没有返回托管画布素材");
...
function readArtifactUri(job: CanvasJob): string | null {
    const direct = job.result?.artifactUri ?? job.result?.storageKey ?? job.result?.uri;
    if (typeof direct === "string" && direct.startsWith("canvas-artifact:")) return direct;
    const patch = job.result_patch?.nodePatch;
    const metadata = patch?.metadata && typeof patch.metadata === "object" && !Array.isArray(patch.metadata) ? patch.metadata as Record<string, unknown> : {};
...
function readJobError(job: CanvasJob): string {
    const value = job.error?.message ?? job.error?.error;
    return typeof value === "string" ? value : "分段视频生成失败";
}

function prioritizeContinuationFrame(referenceAssetIds: string[], continuationFrameArtifactUri: string | null, configuredLimit: unknown): string[] {
    const references = [...new Set(referenceAssetIds.filter((value) => typeof value === "string" && value.trim()))];
    if (!continuationFrameArtifactUri) return references;
