...
import type { UniversalRemakeRun, UniversalRunCandidate } from "./types.js";

const MEDIA_BUCKET = "gouyingai-media";

export function createLocalUniversalCompositionPort(store: LocalStore): UniversalCompositionPort {
    return {
...
export function createSupabaseUniversalCompositionPort(admin: SupabaseClient): UniversalCompositionPort {
    return {
        async extractContinuationFrame(run, candidate, segmentArtifactUri) {
            const directory = await mkdtemp(join(tmpdir(), "gouyingai-continuation-"));
            try {
                const result = await extractUniversalContinuationFrame({
                    ...
                    resolveArtifact: async ({ userId, canvasId, uri }) => {
                        ...
                        if (!isOwnedCanvasArtifactPath(userId, canvasId, storagePath)) return null;
...
function toCompositionInput(run: UniversalRemakeRun, candidate: UniversalRunCandidate, uris: string[]) {
    ...
    return {
        ...
        segments: candidate.manifest.segments.map((segment, index) => ({
            ...
            boundaryAfter: (boundaries[index] || (index === candidate.manifest.segments.length - 1 ? "none" : "hard-cut")) as "hard-cut" | "transition" | "semantic" | "continuous" | "none",
            hasAudio: true,
        })),
    };
}
