...
import type { UniversalRemakeRun, UniversalRemakeRunCreateInput, UniversalRemakeRunRepository } from "./types.js";

export function createLocalUniversalRemakeRepository(store: LocalStore): UniversalRemakeRunRepository {
    return {
...
export function createSupabaseUniversalRemakeRepository(admin: SupabaseClient): UniversalRemakeRunRepository {
    return {
        async create(userId, input) {
            ...
            if (!error && data) return data as UniversalRemakeRun;
            if (error?.code === "23505") {
        ...
        async get(userId, runId) {
            ...
            return (data as UniversalRemakeRun | null) || null;
        },
        ...
        async list(userId, canvasId) {
            ...
            if (error) throw new Error(error.message);
            return (data || []) as UniversalRemakeRun[];
        },
        async listActive() {
            ...
            if (queued.error || running.error) throw new Error(queued.error?.message || running.error?.message);
            return ([...(queued.data || []), ...(running.data || [])] as UniversalRemakeRun[]).sort((left, right) => left.created_at.localeCompare(right.created_at));
        },
        ...
        async updateCandidate(runId, candidateIndex, patch) {
            ...
            const run = current as UniversalRemakeRun;
