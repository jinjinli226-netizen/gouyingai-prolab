...
import { CanvasJobRunner, type CanvasJobHandler } from "./canvas-job-runner.js";
...
function durableJobHandler(handlers: Record<"video" | "text" | "image" | "audio" | "analysis" | "plan", CanvasJobHandler>): CanvasJobHandler {
    const quality = createViralQualityJobHandler();
    return async (context) => {
        if (context.job.kind === "video" || context.job.kind === "viral-video") return handlers.video(context);
        if (context.job.kind === "viral-quality") return quality(context);
        if (context.job.kind === "viral-analysis") return handlers.analysis(context);
        if (context.job.kind === "viral-plan") return handlers.plan(context);
        if (context.job.kind === "text" || context.job.kind === "image" || context.job.kind === "audio") return handlers[context.job.kind](context);
        throw new Error(`Gateway 尚未注册 ${context.job.kind} 任务处理器`);
    };
}

function registerRunnerShutdown(runner: CanvasJobRunner, server: ReturnType<ReturnType<typeof createLocalGatewayApp>["listen"]>, coordinator: ViralBatchCoordinator) {
    let stopping = false;
