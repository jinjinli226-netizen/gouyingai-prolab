...
export function startHttpServer() {
    ...
    app.post("/canvas/run-event", (req, res) => res.json({ ok: true, result: session.resolveRunEvent(req.body || {}) }));
    ...
    app.listen(port, "127.0.0.1", () => {
        console.log("ProLab Agent");
        console.log(`Local URL: ${config.url}`);
        console.log(`Connect token: ${config.token}`);
        console.log("Codex MCP: codex mcp add prolab -- npx -y @proapi-hub/canvas-agent mcp");
    });
}
