...
export const toolInputSchemas = {
    ...
    canvas_get_capabilities: z.object({ canvasId: z.string().optional() }).passthrough(),
    ...
    canvas_create_run: z.object({ canvasId: z.string(), inputs: recordSchema, dryRun: z.boolean() }),
    canvas_start_run: z.object({ canvasId: z.string(), runId: z.string() }),
    canvas_get_run: z.object({ canvasId: z.string(), runId: z.string() }),
    canvas_cancel_run: z.object({ canvasId: z.string(), runId: z.string() }),
    canvas_list_outputs: z.object({ canvasId: z.string(), runId: z.string() }),
    canvas_get_output_download: z.object({ canvasId: z.string(), outputId: z.string() }),
} satisfies Record<ToolName, z.AnyZodObject>;

export const toolDescriptions: Record<ToolName, string> = {
    canvas_get_state: "读取当前网页画布的节点、连线、选区和视口。",
    canvas_get_capabilities: "读取 Canvas Agent 对外提供的稳定工具和独立运行协议版本。",
    ...
    canvas_create_run: "从当前母版建立独立运行实例；dryRun 只校验输入、输出和运行节点，不启动生成。",
    canvas_start_run: "启动已创建的独立运行实例；真实生成仍应由调用方先完成费用与内容确认。",
    canvas_get_run: "查询独立运行实例状态。",
    canvas_cancel_run: "取消独立运行实例并通知已连接网页停止对应生成。",
    canvas_list_outputs: "列出一次运行产生的全部文本、图片、视频、音频或文件产物。",
    canvas_get_output_download: "获取指定运行产物的下载或安全引用元数据。",
};
