...
import { CanvasRunRegistry } from "./canvas-runs.js";
...
export class CanvasSession {
    ...
    updateState(body: unknown, clientId?: string) {
        ...
        this.runs.updateCanvas(this.canvasState);
    }

    resolveRunEvent(body: { runId?: string; status?: string; runCanvasId?: string; error?: string }) {
        if (!body.runId) throw new Error("运行事件缺少 runId");
        if (body.status === "running" && body.runCanvasId) return this.runs.acknowledgeStart(body.runId, body.runCanvasId);
        if (body.status === "failed") return this.runs.fail(body.runId, body.error || "网页无法建立独立运行副本");
        throw new Error(`未知运行事件状态：${String(body.status)}`);
    }
    ...
    async callTool(name: unknown, rawInput: unknown) {
        ...
        if (tool === "canvas_get_capabilities") return { protocolVersion: "1.0", tools: [...toolNames] };
        if (tool === "canvas_create_run") {
            if (!this.clients.size || !this.canvasState) throw new Error("当前没有已连接画布");
            return this.runs.create(this.canvasState, input as { canvasId: string; inputs: Record<string, unknown>; dryRun: boolean });
        }
        if (tool === "canvas_start_run") {
            const data = input as { canvasId: string; runId: string };
            const run = this.runs.getInternal(data.runId);
            if (run.templateCanvasId !== data.canvasId) throw new Error("运行实例不属于当前画布");
            if (!this.clients.size || this.canvasState?.projectId !== data.canvasId) throw new Error("母版画布当前未连接，不能启动独立运行");
            const event = this.runs.startEvent(data.runId);
            if (event) this.sendToCurrentCanvas("run_start", event);
            return this.runs.get(data.runId);
        }
        if (tool === "canvas_get_run") return this.runs.get(String(input.runId || ""));
        if (tool === "canvas_cancel_run") {
            const run = this.runs.getInternal(String(input.runId || ""));
            const result = this.runs.cancel(run.runId);
            if (run.runCanvasId && this.canvasState?.projectId === run.runCanvasId) this.sendToCurrentCanvas("run_cancel", { runId: run.runId, runCanvasId: run.runCanvasId });
            return result;
        }
        if (tool === "canvas_list_outputs") return { outputs: this.runs.listOutputs(String(input.runId || "")) };
        if (tool === "canvas_get_output_download") {
            const outputId = String(input.outputId || "");
            const reference = this.runs.outputReference(outputId);
            try {
                return this.runs.download(reference.runId, outputId);
            } catch (error) {
                if (!(error instanceof Error) || !/网页解析/.test(error.message)) throw error;
                if (!reference.runCanvasId || this.canvasState?.projectId !== reference.runCanvasId) throw new Error("产物所在的独立运行画布当前未连接，无法解析下载地址");
                const resolved = await this.requestCanvasTool("canvas_resolve_output", { outputId, nodeId: reference.nodeId });
                return { outputId, ...((resolved && typeof resolved === "object" ? resolved : {}) as Record<string, unknown>) };
            }
        }
    ...
    private sendToCurrentCanvas(type: string, payload: unknown) {
    ...
    private async requestCanvasTool(name: string, input: Record<string, unknown>) {
