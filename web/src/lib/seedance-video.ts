import { modelOptionName, resolveModelRequestConfigForCapability, type AiConfig } from "@/stores/use-config-store";
...
export function isSeedanceVideoConfig(config: AiConfig | Pick<AiConfig, "model" | "videoModel" | "baseUrl">) {
    const requestConfig = "channels" in config ? resolveModelRequestConfigForCapability(config, "video") : config;
