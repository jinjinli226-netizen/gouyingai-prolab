 ...
 */
export const IMAGE_MODEL_TYPES: ImageModelType[] = ["midjourney", "gemini", "flux", "dalle", "doubao", "gpt4o-image", "gpt-image", "sora-image", "grok-image", "qwen-image", "z-image", "riverflow", "agnes-image", "koukoutu"];
 ...
 */
export const CHAT_MODEL_TYPES: ChatModelType[] = ["gpt", "claude", "gemini-chat", "deepseek", "qwen-chat", "grok", "llama", "moonshot", "glm", "doubao-chat", "minimax", "hunyuan", "mixtral", "phi"];
 ...
 */
export const VIDEO_MODEL_TYPES: VideoModelType[] = ["jimeng-video", "veo", "sora", "kling", "grok-video", "agnes-video"];
...
export const CHAT_MODEL_MATCH_RULES: { type: ChatModelType; patterns: RegExp[] }[] = [
    ...
    {
        type: "minimax",
        patterns: [/^minimax/i, /^MiniMax/],
    },
    {
        type: "hunyuan",
        patterns: [/^hunyuan/i, /混元/],
    },
    {
        type: "mixtral",
        patterns: [/^mixtral/i],
...
export const LOCAL_UPSTREAM_PROXY_PREFIX = "/__gouyingai_upstream";

export function browserProxyBaseUrl(baseUrl: string): string {
    const normalized = normalizeBaseUrl(baseUrl);
    if (typeof window === "undefined") return normalized;
    if (window.location.hostname !== "localhost" && window.location.hostname !== "127.0.0.1") return normalized;
    try {
        const url = new URL(normalized);
        if (url.protocol !== "http:" && url.protocol !== "https:") return normalized;
        const hostname = url.hostname.toLowerCase().replace(/^\[|\]$/g, "");
        if (hostname === "localhost" || hostname === "127.0.0.1" || hostname === "::1") return normalized;
        return `${window.location.origin}${LOCAL_UPSTREAM_PROXY_PREFIX}/${encodeURIComponent(url.origin)}${url.pathname.replace(/\/+$/, "")}`;
    } catch {
        return normalized;
    }
}
