...
export function buildDalleEndpointBody(opts: BuildBodyOpts): Record<string, any> {
  ...
  if (family !== 'agnes-image' && family !== 'gpt-image') body.response_format = responseFormat
  ...
  switch (family) {
    ...
    case 'gpt-image': {
      ...
      if (p.size) body.size = normalizeSize(p.size)
      else if (p.aspectRatio) body.size = openAiGptImageSizeFromRatio(p.aspectRatio)
      if (p.quality === 'standard') body.quality = 'medium'
      else if (p.quality === 'hd') body.quality = 'high'
      else if (p.quality) body.quality = p.quality
...
function openAiGptImageSizeFromRatio(ratio: string): string {
