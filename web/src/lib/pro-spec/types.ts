...
export type ImageModelType =
    ...
    | "riverflow" // Sourceful Riverflow（OpenRouter，chat 输出图像）
    | "agnes-image" // Agnes 绘图（agnes-image-2.0-flash / 2.1-flash）
    | "koukoutu"; // 抠抠图（背景移除）
...
export type VideoModelType =
    ...
    | "agnes-video"; // Agnes 视频（agnes-video-v2.0 等）
...
export type ApiFormat =
    ...
    | "grok-video-chat"; // Grok Video via Chat Completions（newapi 把 grok-imagine-video 暴露为 chat）
...
export interface ImageModelParams {
    ...
    /** 生成数量：1-10 */
    n?: number;
    /** 随机种子（豆包 / Imagen 4 / Seedream / Flux 共用；-1 = 随机，0–4294967295 = 固定） */
    seed?: number;
    /** 质量（DALL-E 3: standard/hd，GPT Image: high/medium/low，Grok: 1k/2k） */
    quality?: "standard" | "hd" | "high" | "medium" | "low" | "1k" | "2k";
    /** 风格（DALL-E 3） */
    style?: "vivid" | "natural";
    /** 提示词相关度：1-10（豆包） */
    ...
    /** 输出压缩率 0-100，仅 jpeg/webp 生效（GPT Image） */
    outputCompression?: number;
    /** 内容审核级别（GPT Image） */
    moderation?: "auto" | "low";
    /** 输入保真度（GPT Image 图生图） */
    inputFidelity?: "low" | "high";
    /** 生成数量（Imagen 4: 1-4 / Seedream: 1-6 / Grok: 1-10 / Qwen: 1-4） */
    sampleCount?: number;
    /** Qwen / Wan 2.7：提示词增强（自动翻译/扩写） */
    promptExtend?: boolean;
    /** Wan 2.7：thinking 模式（深度推理） */
 ...
 * Veo 视频参数
 *
 * 官方支持时长档：4 / 6 / 8 秒（含 veo-3.1 / veo-3.1-fast / veo-3.1-ref）
 * 官方支持分辨率：720p / 1080p / 4k
 */
export interface VeoVideoParams {
    /** 宽高比：16:9, 9:16 */
    aspectRatio?: string;
    /** 时长（秒）：4 / 6 / 8。Provider 会自动转字符串提交 */
    duration?: number;
    /** 分辨率档：720p / 1080p / 4k */
    quality?: "low" | "standard" | "sd" | "720p" | "1080p" | "4k";
    /** 是否生成原生音频（Veo 3+ 支持） */
 ...
 * Sora 视频参数
 *
 * 官方支持时长档：**4 / 8 / 12 / 16 / 20 秒**（含 sora-2 / sora-2-pro）
 * 分辨率（按模型）：
 *   - sora-2：仅 720p（`1280x720` / `720x1280`）
 *   - sora-2-pro：720p + **1024p**（`1792x1024` / `1024x1792`）+ 1080p（`1920x1080` / `1080x1920`）
 */
export interface SoraVideoParams {
    /** 方向（替代宽高比） */
    orientation?: "portrait" | "landscape";
    /** 兼容旧字段（small=720p、large=1080p），新代码请用 quality */
    size?: "small" | "large";
    /** 分辨率档：720p / 1024p（仅 Pro）/ 1080p（仅 Pro） */
    quality?: "low" | "standard" | "sd" | "720p" | "1024p" | "1080p";
    /** 时长（秒）：4 / 8 / 12 / 16 / 20。Provider 会自动转字符串提交 */
    duration?: number;
    /** 水印 */
    watermark?: boolean;
    /** 隐私模式 */
 ...
 * Kling 视频参数
 *
 * 官方支持时长：**3–15 秒任意整数**（含 kling-3.0 / kling-o3）；
 * 固定时长封装：kling-5s（锁 5s）、kling-10s（锁 10s）
 * 官方支持分辨率：720p / 1080p / 4k
 */
export interface KlingVideoParams {
    /** 宽高比：16:9, 9:16, 1:1 等 */
    aspectRatio?: string;
    /** 时长（秒）：3–15 整数（或 5/10 固定档）。Provider 会自动转字符串提交 */
    duration?: number;
    /** 分辨率档：720p / 1080p / 4k */
    quality?: "low" | "standard" | "sd" | "720p" | "1080p" | "4k";
    /** 负面提示词 */
    ...
    /** 是否生成原生音频（Kling 3.0+） */
    sound?: boolean;
    /** Prompt 遵循度：0–1，默认 0.5 */
    cfgScale?: number;
}

/**
 * Grok Video 参数（grok-imagine-video / grok-imagine1.5-video）
 *
 * Grok Imagine 协议：POST /v1/videos + application/json
 * 字段：seconds(数字)/size/images（URL 或 Data URI，最多 7 张）
 *
 * 时长档：6 / 10 / 12 / 16 / 20
 * 画幅：720x1280 / 1280x720 / 1024x1024 / 1024x1792 / 1792x1024
 * 输出清晰度：固定 720p 级
 * `resolutionName` / `preset` 只供旧的 grok-video-chat 兼容接口使用，
 * 不会发送到 grok-imagine-video。
 */
export interface GrokVideoParams {
    /** 时长（秒）：6 / 10 / 12 / 16 / 20 */
    duration?: number;
    /** 画幅尺寸：720x1280 / 1280x720 / 1024x1024 / 1024x1792 / 1792x1024 */
    size?: string;
    /** 旧 grok-video-chat 接口的分辨率档；Grok Imagine 不使用 */
    resolutionName?: string;
    /** 旧 grok-video-chat 接口的风格预设；Grok Imagine 不使用 */
    preset?: string;
}

/**
 * Agnes 视频参数（agnes-video-v2.0）
 *
 * 协议：POST /v1/videos（OpenAI Video 兼容）
 * 实测上游忽略 seconds/size 等输入字段，固定输出 5s · 1280x704。
 * 暂无可调参数；保留空接口以便未来上游开放参数时扩展。
 */
 ...
 * - 使用场景：任务创建、表单提交
 */
export type ModelParams = ImageModelParams | JimengVideoParams | VeoVideoParams | SoraVideoParams | KlingVideoParams | GrokVideoParams | AgnesVideoParams;
...
export function isAbortError(error: unknown): boolean {
    ...
    if (error && typeof error === "object") {
        ...
        return e.name === "AbortError" || e.cause?.name === "AbortError" || e.message?.includes("aborted") || e.message?.includes("abort") || false;
    }
    return false;
}
