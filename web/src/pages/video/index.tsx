...
import { isOmniV2VModel, isOmniVideoModel, normalizeOmniAspectRatio, omniVideoReferenceError, omniVideoSettingsSummary, OMNI_REFERENCE_LIMITS } from "@/lib/omni-video";
...
const LOG_STORE_KEY = "gouyingai:video_generation_logs";
const logStore = localforage.createInstance({ name: "gouyingai", storeName: "video_generation_logs" });
...
export default function VideoPage() {
    ...
    const omni = isOmniVideoModel(modelName);
    const omniSummary = omni ? omniVideoSettingsSummary(effectiveConfig.size) : null;
    const omniV2V = isOmniV2VModel(modelName);
    const referenceLimits = omni
        ? {
              images: omniV2V ? 0 : OMNI_REFERENCE_LIMITS.images,
              videos: omniV2V ? OMNI_REFERENCE_LIMITS.videos : 0,
              audios: 0,
              imageMaxBytes: OMNI_REFERENCE_LIMITS.imageMaxBytes,
              videoMaxBytes: OMNI_REFERENCE_LIMITS.videoMaxBytes,
              audioMaxBytes: 0,
          }
    ...
    const addReferences = async (files?: FileList | null) => {
        ...
        if (!referenceLimits.images && selectedFiles.some((file) => file.type.startsWith("image/"))) message.warning("当前模型不支持参考图");
        if (!referenceLimits.videos && selectedFiles.some((file) => file.type.startsWith("video/"))) message.warning("当前模型不支持参考视频");
        if (!referenceLimits.audios && selectedFiles.some(isSupportedAudioFile)) message.warning("当前模型不支持参考音频");
        const imageFiles = selectedFiles.filter((file) => referenceLimits.images && file.type.startsWith("image/") && file.size <= referenceLimits.imageMaxBytes).slice(0, Math.max(0, referenceLimits.images - references.length));
        const videoFiles = selectedFiles.filter((file) => referenceLimits.videos && file.type.startsWith("video/") && file.size <= referenceLimits.videoMaxBytes).slice(0, Math.max(0, referenceLimits.videos - videoReferences.length));
        const audioFiles = selectedFiles.filter((file) => referenceLimits.audios && isSupportedAudioFile(file) && file.size <= referenceLimits.audioMaxBytes).slice(0, Math.max(0, referenceLimits.audios - audioReferences.length));
        if (referenceLimits.images && selectedFiles.some((file) => file.type.startsWith("image/") && file.size > referenceLimits.imageMaxBytes)) message.warning(`已忽略超过 ${formatBytes(referenceLimits.imageMaxBytes)} 的参考图`);
        if (referenceLimits.videos && selectedFiles.some((file) => file.type.startsWith("video/") && file.size > referenceLimits.videoMaxBytes)) message.warning(`已忽略超过 ${formatBytes(referenceLimits.videoMaxBytes)} 的参考视频`);
        if (referenceLimits.audios && selectedFiles.some((file) => isSupportedAudioFile(file) && file.size > referenceLimits.audioMaxBytes)) message.warning(`已忽略超过 ${formatBytes(referenceLimits.audioMaxBytes)} 的参考音频`);
    ...
    const buildRequestSnapshot = () => {
        ...
        const omniReferenceError = omniVideoReferenceError(modelName, references.length, videoReferences.length, audioReferences.length);
        if (omniReferenceError) {
            message.error(omniReferenceError);
            return null;
        }
    ...
    const pollGenerationLog = async (log: GenerationLog, configOverride?: AiConfig) => {
        ...
        try {
            for (let attempt = 0; attempt < 120; attempt += 1) {
                ...
                await delay(log.task.provider === "omni" ? 8000 : log.task.provider === "seedance" ? 5000 : 2500);
            }
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : "生成失败";
                        ...
                        <div className="mt-6 space-y-5">
                            ...
                            <div className="min-w-0">
                                <div className="mb-2 flex items-center justify-between gap-3">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button size="small" icon={<Upload className="size-3.5" />} disabled={!referenceLimits.images} onClick={() => fileInputRef.current?.click()}>
                                            上传
                                        </Button>
                                    </div>
                                </div>
                                <div className="hover-scrollbar hover-scrollbar-hint flex min-h-24 w-full min-w-0 max-w-full gap-2 overflow-x-scroll overflow-y-hidden rounded-lg border border-dashed border-stone-300 p-2 pb-3 overscroll-x-contain dark:border-stone-700">
                                    ...
                                    {!references.length ? <div className="flex min-w-full items-center justify-center text-sm text-stone-500">{referenceLimits.images ? `暂无参考图，最多 ${referenceLimits.images} 张` : "当前模型不支持参考图"}</div> : null}
                                </div>
                            </div>
                            ...
                            <div className="min-w-0">
                                <div className="mb-2 flex items-center justify-between gap-3">
                                    ...
                                    <Button size="small" icon={<Upload className="size-3.5" />} disabled={!referenceLimits.videos} onClick={() => fileInputRef.current?.click()}>
                                        上传
                                    </Button>
                                </div>
                                <div className="hover-scrollbar hover-scrollbar-hint flex min-h-24 w-full min-w-0 max-w-full gap-2 overflow-x-scroll overflow-y-hidden rounded-lg border border-dashed border-stone-300 p-2 pb-3 overscroll-x-contain dark:border-stone-700">
                                    ...
                                    {!videoReferences.length ? <div className="flex min-w-full items-center justify-center text-sm text-stone-500">{referenceLimits.videos ? `暂无参考视频，最多 ${referenceLimits.videos} 个` : "当前模型不支持参考视频"}</div> : null}
                                </div>
                            </div>
                            ...
                            <div className="min-w-0">
                                ...
                                <div className="hover-scrollbar hover-scrollbar-hint flex min-h-24 w-full min-w-0 max-w-full gap-2 overflow-x-scroll overflow-y-hidden rounded-lg border border-dashed border-stone-300 p-2 pb-3 overscroll-x-contain dark:border-stone-700">
                                    ...
                                    {!audioReferences.length ? <div className="flex min-w-full items-center justify-center text-center text-sm text-stone-500">{referenceLimits.audios ? `暂无参考音频，最多 ${referenceLimits.audios} 个，单个 ${formatBytes(referenceLimits.audioMaxBytes)} 内` : "当前模型不支持参考音频"}</div> : null}
                                </div>
                            </div>
                            ...
                            <div className="flex min-w-0 items-center justify-between gap-3 rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <div className="min-w-0">
                                    ...
                                    <div className="truncate text-xs text-stone-500 dark:text-stone-400">
                                        {modelOptionSourceLabel(effectiveConfig, model)} · {omniSummary ? `${omniSummary.resolution} · ${omniSummary.ratio} · ${omniSummary.seconds}` : `${normalizeResolution(effectiveConfig.vquality)}p · ${videoSizeLabel(effectiveConfig.size)} · ${normalizeVideoSeconds(effectiveConfig.videoSeconds)}s`}
                                    </div>
                                </div>
...
function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    ...
    const omni = isOmniVideoModel(modelOptionName(model));
    return {
        ...config,
        model,
        videoModel: model,
        size: omni ? normalizeOmniAspectRatio(config.size) : seedance ? normalizeSeedanceRatio(config.size) : normalizeVideoSize(config.size),
        videoSeconds: omni ? "10" : normalizeVideoSeconds(config.videoSeconds),
        vquality: omni ? "720" : normalizeResolution(config.vquality),
