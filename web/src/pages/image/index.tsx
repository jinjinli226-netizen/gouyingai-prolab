...
const LOG_STORE_KEY = "gouyingai:image_generation_logs";
const RESULT_ACTION_BUTTON_CLASS = "min-w-0 px-1.5 [&_.ant-btn-icon]:shrink-0 [&>span:last-child]:min-w-0 [&>span:last-child]:truncate";
const logStore = localforage.createInstance({ name: "gouyingai", storeName: "image_generation_logs" });
...
export default function ImagePage() {
    ...
    const generate = async () => {
        ...
        try {
            const logImages = await Promise.all(successImages.map(persistGeneratedImage));
            saveLog(
                buildLog({
                    prompt: text,
                    model,
