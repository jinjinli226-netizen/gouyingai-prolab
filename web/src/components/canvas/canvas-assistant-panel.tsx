...
import { modelOptionName, modelOptionSearchText, modelOptionSourceLabel, normalizeRequestedModelOption, selectableModelsByCapability, useConfigStore, useEffectiveConfig, type AiConfig } from "@/stores/use-config-store";
...
import { CanvasLocalAgentPanel, type ExternalCanvasRunCancel, type ExternalCanvasRunStart } from "./canvas-local-agent-panel";
...
export function CanvasAssistantPanel({ nodes, selectedNodeIds, snapshot, sessions, activeSessionId, onSelectNodeIds, onSessionsChange, onApplyOps, canUndoOps, onUndoOps, onPrepareExternalRun, onActivateExternalRun, onCancelExternalRun, onResolveExternalOutput, onPasteImage, autoConnectLocal, closing, onCollapse }: CanvasAssistantPanelProps) {
                    ...
                    {view === "history" ? (
                        ...
                        <div className="flex h-full flex-col items-center justify-center px-1 text-center">
                            <div className="relative font-serif text-4xl font-bold italic tracking-normal" style={{ color: theme.node.text }}>
                                <span>GouYingAi</span>
                                <DiaTextReveal className="absolute inset-0" colors={["#22d3ee", "#67e8f9", "#e5e7eb"]} textColor="transparent" duration={1.8} startOnView={false} text="GouYingAi" />
                            </div>
    ...
    return (
        ...
        >
            ...
            >
                ...
                <CanvasLocalAgentPanel embedded snapshot={snapshot} canUndoOps={canUndoOps} onApplyOps={onApplyOps} onUndoOps={onUndoOps} onPrepareExternalRun={onPrepareExternalRun} onActivateExternalRun={onActivateExternalRun} onCancelExternalRun={onCancelExternalRun} onResolveExternalOutput={onResolveExternalOutput} autoConnect={autoConnectLocal} />
            </motion.aside>
        </motion.div>
    );
}
    ...
    return (
        <Select value={current} onValueChange={onChange}>
            <SelectTrigger
                ...
                title={current ? modelOptionSearchText(config, current) : "选择文本模型"}
                onMouseDown={(event) => event.stopPropagation()}
                onPointerDown={(event) => event.stopPropagation()}
            >
                <AgentModelIcon model={current} />
                <span className="min-w-0 truncate">{current ? modelOptionName(current) : "选择文本模型"}</span>
                {current ? <span className="hidden max-w-20 shrink-0 truncate opacity-55 sm:inline">{modelOptionSourceLabel(config, current)}</span> : null}
            </SelectTrigger>
            <SelectContent
                data-canvas-no-zoom
                        ...
                        <SelectItem key={model} value={model} textValue={modelOptionSearchText(config, model)}>
                            <span className="flex min-w-0 items-center gap-2">
                                ...
                                <span className="grid min-w-0 flex-1 gap-0.5">
                                    ...
                                    <span className="truncate text-[11px] leading-4 opacity-55">{modelOptionSourceLabel(config, model)}</span>
                                </span>
                            </span>
                        </SelectItem>
                    ))
        ...
        <div className="group/chip relative inline-flex h-8 max-w-[150px] shrink-0 items-center gap-1.5 rounded-lg text-sm" style={{ color: theme.node.text }}>
            {item.dataUrl ? (
                <span className="relative block size-8 shrink-0">
                    {item.type === CanvasNodeType.Video ? <video src={item.dataUrl} muted className="size-8 rounded-lg object-cover" /> : <img src={item.dataUrl} alt="" className="size-8 rounded-lg object-cover" />}
...
function assistantImageReferenceLabel(references: CanvasAssistantReference[], index: number) {
    if (references[index]?.type !== CanvasNodeType.Image || !references[index]?.dataUrl) return undefined;
    const imageIndex = references.slice(0, index + 1).filter((item) => item.type === CanvasNodeType.Image && item.dataUrl).length - 1;
...
function defaultGenerationModel(config: AiConfig, mode: "text" | "image" | "video" | "audio") {
    if (mode === "image") return config.imageModel;
    if (mode === "video") return config.videoModel;
    if (mode === "audio") return config.audioModel;
    return config.textModel;
}

function resolveGenerationModel(config: AiConfig, mode: "text" | "image" | "video" | "audio", model?: string) {
    return normalizeRequestedModelOption(config, model, defaultGenerationModel(config, mode));
}
...
async function buildToolAgentMessages(snapshot: CanvasAgentSnapshot, history: CanvasAssistantMessage[], userMessage: CanvasAssistantMessage): Promise<ResponseInputMessage[]> {
    ...
    const referenceContent = await assistantReferencesToContent(refs);
    return [
        ...
        {
            role: "user",
            content: [...referenceContent, { type: "text", text: `当前画布：${JSON.stringify(compactSnapshot(snapshot))}\n\n用户需求：${userMessage.text}` }],
        },
    ];
}
