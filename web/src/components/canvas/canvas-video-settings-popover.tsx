...
import { isOmniVideoModel, omniVideoSettingsSummary } from "@/lib/omni-video";
...
export function CanvasVideoSettingsPopover({ config, onConfigChange, buttonClassName, placement = "topLeft" }: CanvasVideoSettingsPopoverProps) {
    ...
    const omniSummary = isOmniVideoModel(modelOptionName(config.videoModel)) ? omniVideoSettingsSummary(config.size) : null;
            ...
            <span ref={buttonRef} className="inline-flex min-w-0">
                <Button size="small" type="text" className={buttonClassName || "!h-8 !max-w-[170px] !justify-start !rounded-full !px-2.5"} style={{ background: theme.node.fill, color: theme.node.text }} icon={<Settings2 className="size-3.5" />} onClick={() => setOpen((current) => !current)}>
                    <span className="truncate">
                        {omniSummary ? `${omniSummary.resolution} · ${omniSummary.ratio} · ${omniSummary.seconds}` : `${videoResolutionLabel(config.vquality)} · ${videoSizeLabel(config.size)} · ${videoSecondsLabel(config.videoSeconds)}`}
                    </span>
                </Button>
            </span>
            {panel}
