...
import { CircleDot, Eraser, FolderOpen, Grid2x2, Group, Hand, Image as ImageIcon, Info, Moon, Music2, Palette, Redo2, ScanLine, Settings2, Square, Sun, Trash2, Type, Undo2, Upload, Video } from "lucide-react";
...
export function CanvasToolbar({
    ...
    onOpenMyAssets,
    onOpenViralVideoRemake,
    onOpenUniversalRemake,
}: {
    ...
    onShowAllConnectionsChange: (show: boolean) => void;
    onOpenMyAssets: () => void;
    onOpenViralVideoRemake: () => void;
    onOpenUniversalRemake: () => void;
}) {
    ...
    return (
        <div className="pointer-events-none absolute bottom-5 z-50 flex justify-center" style={{ left: 300, right: 16 }}>
            ...
            <div ref={wrapRef} className="thin-scrollbar pointer-events-auto flex h-14 max-w-full items-center gap-1 overflow-x-auto rounded-xl border px-2 shadow-lg backdrop-blur [&>*]:shrink-0" style={dockStyle}>
                ...
                <Divider theme={theme} />
                <ToolbarButton id="tool-viral-remake" label="爆款复刻" hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onOpenViralVideoRemake}>
                    <ScanLine className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-universal-remake" label="通用复刻 Beta" hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onOpenUniversalRemake}>
                    <Video className="size-4.5" />
                </ToolbarButton>
                <Divider theme={theme} />
            ...
            {appearanceOpen ? (
                ...
                >
                    ...
                    <div className="mt-1 flex items-center justify-between gap-3 rounded-lg px-1.5 py-1">
                        <span className="inline-flex min-w-0 items-center gap-1.5 text-[11px] font-medium opacity-65">显示全部依赖</span>
                        <Switch size="small" checked={showAllConnections} onChange={onShowAllConnectionsChange} />
                    </div>
                </div>
            ) : null}
        </div>
...
function toolLabel(id: string) {
    ...
    if (id === "tool-viral-remake") return "爆款复刻";
    if (id === "tool-universal-remake") return "通用复刻 Beta";
