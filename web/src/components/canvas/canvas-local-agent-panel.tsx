...
import { CANVAS_AGENT_STORAGE_KEYS, useCanvasAgentStore, type AgentAttachment, type AgentChatItem, type AgentEventLog, type AgentPanelTab, type AgentPendingToolCall, type AgentThreadSummary } from "@/stores/canvas/use-canvas-agent-store";
...
const AGENT_CONNECT_STEPS = [
    ...
    { title: "手动启动备用", text: "如果自动发现失败，再运行下面命令。", command: "npx -y @proapi-hub/canvas-agent" },
];
...
export type ExternalCanvasRunStart = { runId: string; templateCanvasId: string; inputs: Record<string, unknown> };
export type ExternalCanvasRunCancel = { runId: string; runCanvasId?: string };
type CanvasLocalAgentPanelProps = {
    ...
    onPrepareExternalRun?: (event: ExternalCanvasRunStart) => Promise<{ runCanvasId: string }> | { runCanvasId: string };
    onActivateExternalRun?: (event: ExternalCanvasRunStart & { runCanvasId: string }) => void;
    onCancelExternalRun?: (event: ExternalCanvasRunCancel) => void;
    onResolveExternalOutput?: (input: { outputId: string; nodeId: string }) => Promise<Record<string, unknown>> | Record<string, unknown>;
};

export function CanvasLocalAgentPanel({ snapshot, canUndoOps, collapsed, embedded, headless, autoConnect, onApplyOps, onUndoOps, onPrepareExternalRun, onActivateExternalRun, onCancelExternalRun, onResolveExternalOutput }: CanvasLocalAgentPanelProps) {
    ...
    const onPrepareExternalRunRef = useRef(onPrepareExternalRun);
    const onActivateExternalRunRef = useRef(onActivateExternalRun);
    const onCancelExternalRunRef = useRef(onCancelExternalRun);
    const onResolveExternalOutputRef = useRef(onResolveExternalOutput);
    ...
    useEffect(() => {
        onPrepareExternalRunRef.current = onPrepareExternalRun;
        onActivateExternalRunRef.current = onActivateExternalRun;
        onCancelExternalRunRef.current = onCancelExternalRun;
        onResolveExternalOutputRef.current = onResolveExternalOutput;
    }, [onActivateExternalRun, onCancelExternalRun, onPrepareExternalRun, onResolveExternalOutput]);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        localStorage.setItem(CANVAS_AGENT_STORAGE_KEYS.url, endpoint);
        localStorage.setItem(CANVAS_AGENT_STORAGE_KEYS.token, token);
        ...
        source.addEventListener("run_start", (event) => {
            const data = parseEventData<ExternalCanvasRunStart>(event);
            if (data) void handleExternalRunStart(endpoint, token, data);
        });
        source.addEventListener("run_cancel", (event) => {
            const data = parseEventData<ExternalCanvasRunCancel>(event);
            if (data) onCancelExternalRunRef.current?.(data);
        });
    ...
    const runToolCall = async (endpoint: string, token: string, payload: AgentPendingToolCall) => {
        try {
            const input = (payload.input || {}) as { ops?: CanvasAgentOp[]; outputId?: string; nodeId?: string };
            ...
            const result = payload.name === "canvas_apply_ops"
                ...
                : payload.name === "canvas_resolve_output"
                  ? await onResolveExternalOutputRef.current?.({ outputId: input.outputId || "", nodeId: input.nodeId || "" })
                  : snapshotRef.current;
    ...
    const handleExternalRunStart = async (endpoint: string, token: string, event: ExternalCanvasRunStart) => {
        try {
            if (!onPrepareExternalRunRef.current || !onActivateExternalRunRef.current) throw new Error("当前网页版本不支持独立画布运行");
            const prepared = await onPrepareExternalRunRef.current(event);
            await postRunEvent(endpoint, token, { runId: event.runId, status: "running", runCanvasId: prepared.runCanvasId });
            onActivateExternalRunRef.current({ ...event, runCanvasId: prepared.runCanvasId });
            addEventLog("独立运行已建立", { ...event, inputs: Object.keys(event.inputs), runCanvasId: prepared.runCanvasId });
        } catch (error) {
            const text = error instanceof Error ? error.message : "无法建立独立运行画布";
            await postRunEvent(endpoint, token, { runId: event.runId, status: "failed", error: text }).catch(() => undefined);
            addEventLog("独立运行失败", { runId: event.runId, error: text });
        }
    };
    ...
    const startResize = (event: ReactPointerEvent<HTMLDivElement>) => {
        ...
        const onUp = () => {
            localStorage.setItem(CANVAS_AGENT_STORAGE_KEYS.panelWidth, String(nextWidth));
...
async function postRunEvent(endpoint: string, token: string, body: { runId: string; status: "running" | "failed"; runCanvasId?: string; error?: string }) {
    const res = await fetch(`${endpoint}/canvas/run-event?token=${encodeURIComponent(token)}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!res.ok) throw new Error("Canvas Agent 未接受独立运行状态");
}
