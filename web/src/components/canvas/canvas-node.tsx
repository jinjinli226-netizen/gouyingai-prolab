...
import { canvasVisualRoleStyle, canvasVisualStageStyle } from "@/lib/canvas/canvas-visual-semantics";
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const stageStyle = canvasVisualStageStyle(data.metadata?.visualStage, colorTheme);
    const roleStyle = data.metadata?.visualRole ? canvasVisualRoleStyle(data.metadata.visualRole) : null;
    ...
    useEffect(() => {
        if (!editRequestNonce || data.type !== CanvasNodeType.Text || Boolean(data.metadata?.viralVideoAnalysis) || data.metadata?.viralVideoPromptRole === "template") return;
    ...
    return (
        ...
        >
            <div className="absolute left-3 top-[-30px] z-[65] flex max-w-[calc(100%-24px)] items-center gap-2" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
                {isEditingTitle ? (
                    <input
                        ref={titleInputRef}
                        value={titleDraft}
                    ...
                    <button
                        ...
                        style={{ color: isGroup ? stageStyle.accent : theme.node.text }}
                        title="双击修改节点名称"
                        onDoubleClick={(event) => {
                            event.stopPropagation();
                ...
                {roleStyle && !isGroup ? (
                    <span className="shrink-0 rounded-full border px-2 py-0.5 text-[9px] font-semibold tracking-wide" style={{ color: roleStyle.accent, borderColor: `${roleStyle.accent}55`, background: `${roleStyle.accent}12` }}>
                        {roleStyle.label}
                    </span>
                ) : null}
            </div>
            ...
            <div
                ...
                style={{
                    background: isGroup ? stageStyle.soft : hasImageContent || hasVideoContent ? "transparent" : theme.node.fill,
                    borderColor: isGroup ? (isGroupDropTarget || isActive ? selectionBlue : stageStyle.border) : hasImageContent ? imageBorderColor : isActive ? selectionBlue : isRelated ? theme.node.muted : roleStyle ? `${roleStyle.accent}55` : theme.node.stroke,
                ...
                onDoubleClick={(event) => {
                    ...
                    if (Boolean(data.metadata?.viralVideoAnalysis) || data.metadata?.viralVideoPromptRole === "template") return;
                    event.stopPropagation();
                    setIsEditingContent(true);
                }}
            >
...
function NodeContent(props: NodeContentRendererProps) {
    if (props.node.metadata?.universalRemakeCard && props.renderNodeContent) return props.renderNodeContent(props.node);
    if (props.node.type === CanvasNodeType.Text && (props.node.metadata?.viralVideoAnalysis || props.node.metadata?.viralVideoPromptRole === "template") && props.renderNodeContent) {
        return props.renderNodeContent(props.node);
    }
    if (props.node.type === CanvasNodeType.Config && props.renderNodeContent) return props.renderNodeContent(props.node);
    if (props.isBatchRoot) return <ImageNodeContent {...props} />;
    const loadingTextHasContent = props.node.type === CanvasNodeType.Text && Boolean(props.node.metadata?.content);
    if (props.node.metadata?.status === "loading" && !loadingTextHasContent) return <LoadingContent theme={props.theme} />;
...
function GroupNodeContent({ node, theme, groupChildCount }: NodeContentRendererProps) {
    const colorTheme = useThemeStore((state) => state.theme);
    const stageStyle = canvasVisualStageStyle(node.metadata?.visualStage, colorTheme);
    return (
        <div className="pointer-events-none flex h-full w-full flex-col p-4">
            <div className="flex items-center gap-2 text-sm font-semibold" style={{ color: stageStyle.accent }}>
                <span className="grid size-8 place-items-center rounded-xl" style={{ background: stageStyle.soft, color: stageStyle.accent }}>
            ...
            <div className="mt-3 flex-1 rounded-2xl border border-dashed" style={{ borderColor: stageStyle.border, background: stageStyle.soft }} />
        </div>
    );
}
...
function EmptyImageContent({ node, theme, isBatchRoot, batchCount, batchExpanded, batchOpening, batchRecovering, onToggleBatch }: NodeContentRendererProps) {
    const content = (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3" style={{ color: theme.node.placeholder }}>
            ...
            <span className="max-w-[80%] text-center text-[10px] tracking-[0.12em] opacity-50">{node.metadata?.placeholder || "空图片节点"}</span>
        </div>
    );
    if (isBatchRoot)
        return (
