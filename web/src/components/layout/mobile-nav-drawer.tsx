...
type MobileNavDrawerProps = {
    open: boolean;
    activeToolSlug?: NavigationToolSlug | (typeof adminNavigationTool)["slug"];
    onClose: () => void;
};
            ...
            <div className="space-y-1">
                ...
                {isGatewayConfigured ? (
                    <Link
                        ...
                        className={cn(
                            "flex items-center gap-3 rounded-lg px-3 py-3 text-base transition",
                            activeToolSlug === adminNavigationTool.slug ? "bg-stone-100 font-medium text-stone-950 dark:bg-stone-800 dark:text-stone-100" : "text-stone-600 hover:bg-stone-100 hover:text-stone-950 dark:text-stone-300 dark:hover:bg-stone-800 dark:hover:text-stone-100",
                        )}
                    >
                        <adminNavigationTool.icon className="size-5" />
                        <span>{adminNavigationTool.label}</span>
                    </Link>
                ) : null}
            </div>
        </Drawer>
