...
import { isOmniVideoModel, normalizeOmniAspectRatio } from "@/lib/omni-video";
import { type CanvasTheme } from "@/lib/canvas-theme";
import { gatewayModelCatalogEntry, modelOptionName, type AiConfig } from "@/stores/use-config-store";
import { autoDlH3CanvasModelKind, isAutoDlH3CanvasModel } from "@/lib/canvas/autodl-h3-video-routing";
...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    const model = modelOptionName(config.videoModel);
    ...
    const isGrokImagineVideo = /grok.*imag(?:e|ine).*video/i.test(modelOptionName(config.videoModel));
    const h3Options = gatewayModelCatalogEntry(config.videoModel)?.options;
    const isH3 = isAutoDlH3CanvasModel(h3Options);
    const isH3Auto = autoDlH3CanvasModelKind(h3Options) === "auto";
    const seconds = config.videoSeconds || "6";
    const availableSeconds = isH3 ? [5, 10, 15] : secondOptions;
    ...
    return (
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                {isH3Auto ? (
                    <SettingGroup title="素材方式" color={theme.node.muted}>
                        <div className="grid grid-cols-3 gap-2.5">
                            {[
                                { value: "auto", label: "自动识别" },
                                { value: "first-last", label: "首尾帧" },
                                { value: "lip-sync", label: "口型同步" },
                            ].map((item) => (
                                <OptionPill key={item.value} selected={config.videoInputMode === item.value} theme={theme} onClick={() => onConfigChange("videoInputMode", item.value)}>
                                    {item.label}
                                </OptionPill>
                            ))}
                        </div>
                        <div className="text-[11px] leading-4 opacity-55">自动识别按提示词、图片、音频和时长选择工作流；首尾帧与口型同步必须显式选择。</div>
                    </SettingGroup>
                ) : null}
                <SettingGroup title="清晰度" color={theme.node.muted}>
                    {isGrokImagineVideo ? <div className="text-sm">固定 720p 级清晰度</div> : <div className="grid grid-cols-3 gap-2.5">
                ...
                <SettingGroup title="尺寸" color={theme.node.muted}>
                    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2.5">
                        <DimensionInput prefix="W" value={dimensions.width} disabled={size === "auto" || isGrokImagineVideo} theme={theme} onChange={(value) => updateDimension("width", value)} />
                        <span className="text-lg opacity-45">↔</span>
                        <DimensionInput prefix="H" value={dimensions.height} disabled={size === "auto" || isGrokImagineVideo} theme={theme} onChange={(value) => updateDimension("height", value)} />
                    </div>
                    <div className="grid grid-cols-3 gap-2.5">
                ...
                <SettingGroup title="秒数" color={theme.node.muted}>
                    <div className="grid grid-cols-3 gap-2.5">
                        ...
                        {isGrokImagineVideo ? null : <NumberInput value={seconds} min={1} max={isH3 ? 15 : 20} theme={theme} onChange={(value) => onConfigChange("videoSeconds", value)} />}
                    </div>
                </SettingGroup>
            </div>
        </ImageSettingsTheme>
...
function OmniVideoSettingsPanel({ config, onConfigChange, theme, showTitle, className }: VideoSettingsPanelProps) {
    const ratio = normalizeOmniAspectRatio(config.size);
    return (
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                {showTitle ? <div className="text-lg font-semibold">视频设置</div> : null}
                <SettingGroup title="清晰度" color={theme.node.muted}>
                    <div className="text-sm">固定 720p</div>
                </SettingGroup>
                ...
                <SettingGroup title="时长" color={theme.node.muted}>
                    <div className="text-sm">固定约 10 秒</div>
                </SettingGroup>
            </div>
        </ImageSettingsTheme>
    );
...
export function normalizeVideoResolutionValue(value: string) {
    ...
    if (value === "auto" || value === "high" || value === "medium") return "768";
    return value.replace(/p$/i, "") || "768";
}
