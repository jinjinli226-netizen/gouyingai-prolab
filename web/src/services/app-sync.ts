...
const imageLogStore = localforage.createInstance({ name: "gouyingai", storeName: "image_generation_logs" });
const videoLogStore = localforage.createInstance({ name: "gouyingai", storeName: "video_generation_logs" });
...
async function syncDomain<T>(config: WebdavSyncConfig, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    try {
        ...
        const manifest: DomainManifest<T> = { app: "gouyingai", version: 1, domain: options.key, exportedAt: new Date().toISOString(), data: mergedData, files: uploaded.files };
...
async function readDomainManifest<T>(config: WebdavSyncConfig, domain: DomainKey, emptyData: T): Promise<DomainManifest<T> | null> {
    ...
    if (data.app !== "gouyingai" || data.domain !== domain) throw new Error(`${domain} 同步清单不是当前应用的数据`);
    return {
        app: "gouyingai",
        version: 1,
        domain,
