...
- 保持无状态交付：每次修改时都保持产物是直接可交付的，不包含版本补丁说明、解释为何修改或如何修改等过程描述。
...
- 模型路由必须按显式渠道、模型名和能力严格匹配；配置失效、能力不符或同名歧义时直接报错，禁止自动重绑、跨能力调用或回退到默认模型。
- 爆款复刻只有一种用户模式。原片先生成一份不可变的完整母版；`candidateCount` 始终表示 1–1000 条完整候选视频，不表示镜头变体。拉片分段只用于分析和组织母版，每条候选只能提交一次付费视频生成，禁止按分析分段拆成多次生成。提交前必须通过对象绑定、P0 事件覆盖、时长可行性与费用预检；大批量必须由 Gateway 有界窗口持久调度，禁止在画布创建同等数量的节点。最终视频请求必须携带人物、商品、场景绑定与全片连续性规则。
- 当前画布项目和“我的素材”主要保存在浏览器本地，不要在文档中误写成已支持云同步。
- AI 渠道、API Key、模型发布状态和排序只允许在管理后台维护；前台只读取网关发布目录，禁止恢复浏览器本地渠道编辑或 API 一键接入入口。
