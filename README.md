...
渠道、API Key 和模型目录由本地 Gateway 统一维护；画布、素材和生成记录默认保存在浏览器本地。
...
统一启动器会同时启动 Gateway（8788）和前端（3000），访问 `http://localhost:3000` 即可。

渠道、模型与 API Key 统一在“管理后台”维护；本地数据保存在 `gateway/data/local-store.json`，API Key 以密文保存。

## 数据与持久化

当前版本不需要注册登录，也没有服务端用户数据库。

- 渠道、API Key 与模型配置保存在本地 Gateway；画布、素材、生图记录和视频记录保存在当前浏览器本地。
